...
namespace ndd {
    namespace filter {
        ...
        struct Bucket {
            ...
            uint32_t get_value(size_t index) const { return base_value + deltas[index]; }
            ...
            std::vector<uint8_t> serialize() const {
                ...
                // 3. Count
                std::memcpy(ptr, &count, 2);
                ptr += 2;

                // 4. Deltas
                if(count > 0) {
                    std::memcpy(ptr, deltas.data(), count * 2);
                    ptr += count * 2;
                }
            ...
            static Bucket deserialize(const void* data, size_t len, uint32_t base_val) {
                ...
                if(ptr + 2 > end) {
                    throw std::runtime_error("Bucket corrupt: truncated count");
                }

                // 3. Count
                uint16_t count;
                std::memcpy(&count, ptr, 2);
                ptr += 2;
        ...
        private:
            ...
            void remove_from_buckets(MDBX_txn* txn,
                ...
                if(rc == MDBX_SUCCESS) {
                    ...
                    if(found_key.rfind(field + ":", 0) == 0) {
                        ...
                        if(value >= bucket_base) {
                            Bucket b =
                                    Bucket::deserialize(data.iov_base, data.iov_len, bucket_base);
                            if(b.remove(id)) {
            ...
            void
            add_to_buckets(MDBX_txn* txn, const std::string& field, uint32_t value, ndd::idInt id) {
                MDBX_cursor* cursor;
                mdbx_cursor_open(txn, inverted_dbi_, &cursor);
                ...
                if(rc == MDBX_SUCCESS) {
                    ...
                    if(found_key.rfind(field + ":", 0) == 0) {
                        ...
                        if(value >= target_base
                           && (static_cast<uint64_t>(value) - target_base) <= Bucket::MAX_DELTA) {
                ...
                if(create_new) {
                    ...
                    // Update existing
                    // We must re-fetch current key/data because cursor move might have updated
                    // key/data
                    MDBX_val k{const_cast<char*>(target_key_str.data()), target_key_str.size()};
                    MDBX_val v;
                    ...
                    if(b.ids.size() >= Bucket::MAX_SIZE) {
                        ...
                        while(probe_right < b.deltas.size() && probe_right > 0
                              && b.deltas[probe_right] == b.deltas[probe_right - 1]) {
                            probe_right++;
                        }
                        ...
                        if(probe_right < b.deltas.size()) {
                            ...
                            while(probe_left > 0
                                  && b.deltas[probe_left] == b.deltas[probe_left - 1]) {
                                probe_left--;
                            }
                        ...
                        if(mid_idx == b.deltas.size()) {
                            // Fallback: Just append (overfill) or implement logic to handle
                            // identicals. For now: Append
                            b.add(value, id);
                            auto bytes = b.serialize();
